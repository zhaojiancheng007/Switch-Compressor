...
from switch_nerf.datasets.filesystem_dataset import FilesystemDataset, Mydata
...
class Runner:
    def __init__(self, hparams: Namespace, set_experiment_path: bool = True):
        ...
        # experiments path for logging result
        # self.experiment_path = self._get_experiment_path() if self.is_master else None
        self.model_path =os.path.join(os.getcwd(), 'models') if self.is_master else None
        ...
        self.nerf = get_nerf(hparams, appearance_count=3).to(self.device)
        self.model_parameter_num = count_parameters(self.nerf)
        self.model_structure(self.nerf)
...
########################################## 准备datasets ##########################################
        ...
        # }
                ...
                dataset = Mydata(hparams=self.hparams)
        ...
        if 'RANK' in os.environ:
            ...
            # coord: [Batch_size, 3]
            # gt: [Batch, 3]
            data_loader = DataLoader(dataset, batch_size=self.hparams.batch_size // world_size,  
                                     sampler=sampler, num_workers=self.hparams.data_loader_num_workers, pin_memory=True)
        else:
...
################################ 训练forward ####################################
                        ...
                        metrics, bg_nerf_rays_present = training_step_fn(
                            gt = item['gt'].to(self.device, non_blocking=True),
                            coords = item['coords'].to(self.device, non_blocking=True),
                            image_indices= None
                            )
...
######################################### checkpoint every_1000 ###########################################
                    if train_iterations > 0 and train_iterations % self.hparams.ckpt_interval == 0:
                        ...
                        self._save_checkpoint(tmp_optimizers, scaler, train_iterations, dataset_index,
                                              dataset.get_state() if self.hparams.dataset_type == 'Switch-NeRF dataset' else None)
                    if train_iterations % self.hparams.i_print==0 or train_iterations == self.hparams.train_iterations:
                        ...
                        if self.is_master:
                            print('all_loss:', metrics['all_loss'])
                            print('aux_loss:', moe_l_aux_wt * metrics['gate_loss'])
        ...
        if self.is_master:
            ...
            tmp_optimizers = optimizers
            self._save_checkpoint(tmp_optimizers, scaler, train_iterations, dataset_index,
                                  dataset.get_state() if self.hparams.dataset_type == 'xxxxx' else None)
...
################################ 修改 render_rays ####################################
        ...
        metrics = {}
        photo_loss = F.mse_loss(results[f'sigma'], gt, reduction='mean')
        metrics['loss'] = photo_loss

        if (self.hparams.use_moe or self.hparams.bg_use_moe) and self.hparams.use_balance_loss:
            gate_loss = torch.mean(results[f'gate_loss'])
            metrics[f'gate_loss'] = gate_loss
    ...
    def eval_image(self):
        # self._setup_experiment_dir()
        if not self.hparams.moe_test_batch: # False
            self.set_no_batch(mode=True)
        else:
            self.set_no_batch(mode=False)
