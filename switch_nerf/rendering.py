...
def render_rays(nerf: nn.Module,
                bg_nerf: Optional[nn.Module], # None
                coords: torch.Tensor,
                ...
                get_depth_variance: bool, # False
                get_bg_fg_rgb: bool) -> Tuple[Dict[str, torch.Tensor], bool]: # False
    # N_rays = rays.shape[0]
    ...
    if bg_nerf is not None:
        ...
        for typ in ["fine", "coarse"]:
            if rays_with_bg.shape[0] > 0:
                # save gate loss
                if f'gate_loss_{typ}' in bg_results:
                    results[f'bg_gate_loss_{typ}'] = bg_results[f'gate_loss_{typ}'] 
                
                if f'balance_loss_{typ}' in bg_results:
                    results[f'bg_balance_loss_{typ}'] = bg_results[f'balance_loss_{typ}']
            # save zero gate loss
            else:
                if'gate_loss_{typ}' in bg_results:
                    results[f'bg_gate_loss_{typ}'] = torch.tensor(0.0, device=rays_with_bg.device)

                if f'balance_loss_{typ}' in bg_results:
                    results[f'bg_balance_loss_{typ}'] = torch.tensor(0.0, device=rays_with_bg.device)

    bg_nerf_rays_present = bg_nerf is not None
...
def _get_results(nerf: nn.Module,
    ...
    # hparams.fine_samples = hparams.close_fine_sample
...
def _inference(results: Dict[str, torch.Tensor],
    ...
    # 最终预测sigma时用到 sigma+=sigma_noise
    # if hparams.use_sigma_noise:
    #     if hparams.sigma_noise_std > 0.0:
    #         sigma_noise = torch.randn(len(coords), 1, device=coords.device) * hparams.sigma_noise_std if nerf.training else None
    #     else:
    #         sigma_noise = None
    # else:
    #     sigma_noise = None
    ...
    model_chunk = nerf(coords, sigma_noise=None)   
    results[f'gate_loss'] = model_chunk["extras"]["moe_loss"]
    # results[f'moe_gates'] = model_chunk["extras"]["moe_gates"]
    results[f'sigma'] = model_chunk['outputs']
    ...
    #         #         model_chunk = nerf(typ == 'coarse', xyz_chunk, sigma_noise=sigma_noise, **nerf_kwargs)
    #         #     else:
    ...
    #             else:
    #                 model_chunk = nerf(xyz_chunk, sigma_noise=sigma_noise)
    #         # outchunk是一个列表，每一次调用_inference()函数，往里面填一次{"outputs": outputs,"extras": extras}，
    #         # 一共调用4次，bg_nerf调用一次，在里面fine又调用一次；nerf调用一次，在里面fine又调用一次
    #         out_chunks += [model_chunk]
...
#             results[f'gate_loss'] = torch.cat(gate_loss, 0)
...
# ################ 这里可能需要改？#################
#             results[f'moe_gates'] = moe_gates.view(B, 1, moe_gates.shape[1], moe_gates.shape[2])
...
#             results[f'balance_loss'] = torch.cat(balance_loss, 0)
