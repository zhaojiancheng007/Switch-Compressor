...
class MipEmbedder(nn.Module):
    def __init__(self, num_freqs: int, logscale=True, input_dims=3):
        super(MipEmbedder, self).__init__()
        embed_fns = []
        d = input_dims
        out_dim = 0
        ...
        if logscale:
            freq_bands_y = 2.**torch.linspace(min_freq, max_freq, steps=num_freqs)
            freq_bands_w = 4.**torch.linspace(min_freq, max_freq, steps=num_freqs)
        else:
            freq_bands_y = torch.linspace(2.**min_freq, 2.**max_freq, steps=num_freqs)
            freq_bands_w = torch.linspace(4.**min_freq, 4.**max_freq, steps=num_freqs)
